    ...
    <div className="panel-shell p-4 mt-3">
      ...
      {reservationIds.length === 0 ? (
        ...
        reservationIds.map(reservationId => {
          ...
          const baselineIds = reservationSeats.map(s => s.id);
          const isExpanded = expandedId === reservationId;
          const isEditing = editingId === reservationId;

          const handleEditSeatClick = (seat) => {
            const isCurrentReservationSeat = baselineIds.includes(seat.id);
            if (seat.reserved && !isCurrentReservationSeat) return;
          ...
          const canInteractEditSeat = (seat) => {
            const isCurrentReservationSeat = baselineIds.includes(seat.id);
            return !seat.cooldown && (!seat.reserved || isCurrentReservationSeat);
          };
            ...
            <div key={reservationId} className="reservation-card active-reservation p-4 mb-3">
              ...
              {isEditing && theater && (
                <div className="mt-4">
                  ...
                  <SeatMap
                    theater={theater}
                    baselineSeatIds={baselineIds}
                    draftSeatIds={draftSeatIds}
                    onSeatClick={handleEditSeatClick}
                    canInteractSeat={canInteractEditSeat}
                    loggedIn={true}
                  />
