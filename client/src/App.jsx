...
function AppContent() {
  ...
  */ 
  ...
  function Login(props) {
    if (props.loggedIn) {
      if (props.user?.canDoTotp) {
        ...
        setMessage('Successful login, but you cannot access the admin area (NO TOTP)'); // messaggio che non può accedere all'area admin
        return <Navigate replace to='/' />;
      }
    } else {
  ...
  return (
    <div className={`toast-message d-flex align-items-center gap-3 ${isFail ? 'error' : ''}`}>
      <span>{message}</span>
  ...
  if (loading) {
    return <div className="p-4">Checking session...</div>; // mostra un messaggio di caricamento mentre si verifica la sessione.
  }
    ...
    <Toast message={message} onClose={() => setMessage('')} />
      <Routes>
        ...
        <Route element={<AppLayout loggedIn={loggedIn} user={user} loggedInTotpAdmin={loggedInTotpAdmin} logout={handleLogout} setMessage={setMessage} />}>
          <Route path="/"   element={ 
                              <Col xs={12} >
                                <div className="home-hero d-flex flex-column gap-4">
                                  {theater ? (
                                    <SeatMap
                                      theater={theater}
