    ...
    db.all(sql, [], (err, rows) => {
      ...
      for (const row of rows) {
        ...
        rowsMap.get(row.row_label).push({
          ...
          mine: user ? row.owner_id === user.id : false, // se l'utente è loggato, indica se il posto è prenotato da lui
          cooldown: false,
...
function getSeatsByUser(userId) {
  return new Promise((resolve, reject) => {
    db.all(`
      ...
      WHERE r.owner_id = ? 
      ORDER BY s.row_order, s.seat_number
    `, [userId], (err, rows) => { // Ottiene tutti i posti prenotati da un utente specifico (parametrico userId)
      if (err) reject(err);
      else resolve(rows);
    });
  });
...
function getReservationById(reservationId) {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM reservations WHERE id = ?', [reservationId], (err, row) => { // parametrico reservationId, ottiene i dettagli di una prenotazione specifica
      if (err) reject(err);
